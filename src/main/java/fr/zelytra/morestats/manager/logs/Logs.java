...
package fr.zelytra.morestats.manager.logs;
...
public class Logs {
    ...
    private void right() {
        synchronized (syncObject) {
            if (!Bukkit.getServer().isStopping()) {
                Bukkit.getScheduler().runTaskAsynchronously(MoreStats.getInstance(), () -> {
                    rightTask();
                });
            } else {
                rightTask();
    ...
    private void initLogsFile(String date) {
        ...
        String currentDir = MoreStats.getInstance().getDataFolder().getPath();
        MoreStats.getInstance().getDataFolder().mkdir();
